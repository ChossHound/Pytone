...
class DropDown(Widget):
    ...
    Properties:
     ...
     - options: the list of options to choose from. A List of tuples between
        the name of the option and its ID. The ID does not have to follow the
        index.
     - width: how large the widget is drawn. Calculated from the length of the
     longest option.
    ...
    def __init__(self,
        ...
        self.width: int = len(max(options,
                                  key=lambda x: len(x[0]))[0]) * 4*PIXEL_SCALE + 10*PIXEL_SCALE
        self.index: int = 0
    ...
    def get_rect(self) -> pygame.Rect:
        ...
        if self.open:
            return pygame.Rect(self.position,
                               (self.width, 3*PIXEL_SCALE + len(self.options)*9*PIXEL_SCALE))
        ...
        if self.open:
            for i in range(0, len(self.options)):
                self.font.render_to(self.screen,
                                    (self.position[0] + 2*PIXEL_SCALE,
                                     self.position[1] + 9*PIXEL_SCALE + 9*PIXEL_SCALE*i),
                                    self.options[i][0], TEXT_COLOR)
        else:
            self.font.render_to(self.screen, (self.position[0] + 2*PIXEL_SCALE,
                                              self.position[1] + 9*PIXEL_SCALE),
                                self.options[self.index][0],
                                TEXT_COLOR)
            self.font.render_to(self.screen, (self.position[0] + self.width - 6*PIXEL_SCALE,
                                              self.position[1] + 8*PIXEL_SCALE),
                                "v",
                                BUTTON_COLOR)
